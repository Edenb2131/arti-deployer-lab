...
```
            ...
            └─────┬────────────────────────────────────────────┘
                  ...
                  └── scripts/configure-keycloak.sh   (if Keycloak)
```
...
invoke `docker compose -f compose/art1.yml -f compose/keycloak.yml up`
...
|---|---|---|
| Compose file | `compose/<overlay>.yml` | `compose/keycloak.yml` |
| Per-service config | `config/<overlay>/<file>` | `config/keycloak/realm.json` |
...
| Container | `arti-<name>` for overlays; `<base>` for primary | `arti-keycloak`, `artifactory1` |
| Volume | `arti-deployer_<service>-data` | `arti-deployer_keycloak-data` |
| Network alias | container name | `keycloak`, `artifactory1` |
